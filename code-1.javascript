...
class UI {
  static init() {
    ...
    _0x3d849d.innerHTML = `
            ...
            <div class="mathhg-header">
                Mathhg <span class="mathhg-version">${APP.ver}</span>
            </div>
            <div class="mathhg-content">
                <div class="mathhg-opt">
                ...
                <div class="mathhg-opt" id="speedControlContainer" style="display: none;">
                ...
                <div class="mathhg-credit">by iUnknownBr</div>
            </div>
        `;
    document.body.appendChild(_0x3d849d);
    
    // Adicionar evento de clique ao cabeÃ§alho para encolher/expandir o menu
    const header = document.querySelector('.mathhg-header');
    const content = document.querySelector('.mathhg-content');
    ...
    header.addEventListener('click', () => {
      ...
      localStorage.setItem('mathhg-collapsed', isCollapsed);
    ...
    const wasCollapsed = localStorage.getItem('mathhg-collapsed') === 'true';
    if (wasCollapsed) {
...
async function initApp() {
  try {
    ...
    UI.init();
    Core.init();
    console.log(`Mathhg v${APP.ver} iniciado com sucesso!`);
    sendToast(`ðŸš€ Mathhg v${APP.ver} iniciado!`, 3000);
  } catch (error) {
    console.error("Erro ao inicializar Mathhg:", error);
    sendToast("âš ï¸ Erro ao inicializar Mathhg", 5000);
  }
}
